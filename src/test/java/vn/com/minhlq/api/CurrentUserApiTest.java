...
@Import({ SecurityConfig.class, JacksonConfig.class, ValidationAutoConfiguration.class })
public class CurrentUserApiTest extends TestWithCurrentUser {
    ...
    public void should_get_current_user_with_token() throws Exception {
        ...
        given().header("Authorization", "Token " + token).contentType("application/json").when().get("/user").then()
                .statusCode(200).body("user.email", equalTo(email)).body("user.username", equalTo(username))
    ...
    public void should_get_401_with_invalid_token() throws Exception {
        ...
        when(Optional.of(jwtService.getUsernameFromJWT(eq(invalidToken)))).thenReturn(Optional.empty());
        given().contentType("application/json").header("Authorization", "Token " + invalidToken).when().get("/user")
                .then().statusCode(401);
    }
    ...
    public void should_get_error_if_email_exists_when_update_user_profile() throws Exception {
        ...
        when(userRepository.findByEmail(eq(newEmail)))
                .thenReturn(Optional.of(new User("username", "123", "0911", newEmail)));
        when(userRepository.findByUsername(eq(newUsername))).thenReturn(Optional.empty());

        when(userRepository.findById(eq(user.getId()))).thenReturn(Optional.of(user));

        given().contentType("application/json").header("Authorization", "Token " + token).body(param).when()
                .put("/user").prettyPeek().then().statusCode(422)
    ...
    public void should_get_401_if_not_login() throws Exception {
        given().contentType("application/json").body(new HashMap<String, Object>() {
            {
                put("user", new HashMap<String, Object>());
            }
        }).when().put("/user").then().statusCode(401);
    }
